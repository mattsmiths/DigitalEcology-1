...
int max_recording_iters = 100; //currently set for one minute, per data recording cycle
...
void loop(){
    ...
    while(counter < max_recording_iters){
        ...
        if (!sensorState_2 && lastState_2) {
          Serial.println("New movement on beam 2");
    ...
    File dataFile = SD.open("BlbLgr.csv", FILE_WRITE); //NB you can change the filename, but must be shorter than 8 characters total
